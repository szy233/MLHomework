...
def l1_regularization(model, l1_alpha):
    ...
    for name, parameters in model.named_parameters():
        if name in ["linear1.0.weight", "linear2.0.weight", "linear3.weight", "conv1.weight", "conv2.weight",
                    "conv3.weight", "conv4.weight"]:
...
for epoch in range(epochs):
    ...
    for name, parameters in model.named_parameters():
        if name in ["linear1.0.weight", "linear2.0.weight", "linear3.weight", "conv1.weight", "conv2.weight",
            ...
            w_flatten = parameters.clone().detach().cpu().numpy().reshape(-1)
...
torch.save(model.state_dict(), f'lastest_{seed}.pt')
